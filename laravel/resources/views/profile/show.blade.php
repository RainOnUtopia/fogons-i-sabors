...
@section('content')
    @php
        // El tab actiu es detecta pel paràmetre 'tab'
    ...
    <div class="page-grid-bg profile-show-bg">
        <div class="container profile-show-container">
            ...
            <div
                class="profile-header profile-header-custom mb-4 p-4 position-relative d-flex flex-column align-items-center justify-content-center">
                ...
                <form id="avatarUploadForm" method="POST" action="{{ route('profile.avatar.update') }}"
                    enctype="multipart/form-data" class="mb-3">
                    @csrf
                    @method('PATCH')

                    <label for="avatarInput" class="profile-avatar-label" title="Fes clic per canviar la imatge de perfil">
                        <img src="{{ $user->avatar ? asset('storage/' . $user->avatar) . '?v=' . $user->updated_at?->timestamp : asset('img/user-avatar.svg') }}"
                            alt="Avatar" class="mb-0 profile-avatar-image">
                    </label>

                    <input id="avatarInput" type="file" name="avatar" accept=".png,.jpg,.jpeg,image/png,image/jpeg"
                        class="d-none"
                ...
                <div class="d-flex flex-wrap gap-3 justify-content-center align-items-center mt-2 profile-user-meta">
                    @if($user->city || $user->country)
                        <span><i class="bi bi-geo-alt"></i>
                            {{ collect([$user->city, $user->country])->filter()->join(', ') }}</span>
                    @endif
                        ...
                        <div class="card mb-4 profile-sidebar-card">
                            <div class="card-body">
                                ...
                                <p class="mb-4 profile-about-text">
                                    {{ $user->about_me ?? 'Sense descripció.' }}
                                </p>
                    ...
                    <div class="col-lg-8">
                        ...
                        <ul class="nav nav-tabs mb-4 profile-tabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link {{ $activeProfileTab === 'pantry' ? 'active' : '' }} fw-bold"
                                    id="profile-pantry-tab" data-bs-toggle="tab" data-bs-target="#profile-pantry-pane"
                                    type="button" role="tab" aria-controls="profile-pantry-pane"
                                    aria-selected="{{ $activeProfileTab === 'pantry' ? 'true' : 'false' }}">El meu
                                    rebost</button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link {{ $activeProfileTab === 'favorites' ? 'active' : '' }} fw-bold"
                                    id="profile-favorites-tab" data-bs-toggle="tab" data-bs-target="#profile-favorites-pane"
                                    type="button" role="tab" aria-controls="profile-favorites-pane"
                                    aria-selected="{{ $activeProfileTab === 'favorites' ? 'true' : 'false' }}">Plats
                                    favorits</button>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade {{ $activeProfileTab === 'pantry' ? 'show active' : '' }}"
                                id="profile-pantry-pane" role="tabpanel" aria-labelledby="profile-pantry-tab" tabindex="0">

                                {{-- ── BARRA DE FILTRES DEL REBOST ────────────────────────────── --}}
                                @php
                                    // Preservam tots els paràmetres actuals excepte el que acabem de canviar.
                                    $rExceptSearch = request()->except(['r_search', 'r_page', 'tab']);
                                    $rExceptDifficulty = request()->except(['r_difficulty', 'r_page', 'tab']);
                                    $rExceptSort = request()->except(['r_sort', 'r_direction', 'r_page', 'tab']);
                                @endphp
                                    ...
                                    <form method="GET" class="mb-2 search-profile-form">
                                        ...
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="text" name="r_search" class="form-control search-profile"
                                                placeholder="Cerca per nom o ingredient..." value="{{ $rSearch }}">
                                            @if($rSearch)
                                                <a href="{{ route('profile.show', array_merge($rExceptSearch, ['tab' => 'pantry'])) }}"
                                                    class="btn btn-outline-secondary btn-sm not-filter-button"
                                                    title="Esborra la cerca">
                                                    <i class="bi bi-x-lg "></i>
                                                </a>
                                            @endif
                                            <button type="submit"
                                                class="btn btn-primary btn-sm bi bi-search profile-search-button"></button>
                                        </div>
                                    </form>
                                    ...
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        ...
                                        <form method="GET" class="ms-auto">
                                            ...
                                            <select name="r_sort" class="form-select form-select-sm d-inline-block w-auto"
                                                ...
                                                <option value="created_at" data-dir="desc"
                                                    {{ $rSort === 'created_at' && $rDirection === 'desc' ? 'selected' : '' }}>
                                                    Més recents</option>
                                                <option value="created_at" data-dir="asc"
                                                    {{ $rSort === 'created_at' && $rDirection === 'asc' ? 'selected' : '' }}>
                                                    Més antics</option>
                                                <option value="average_rating" data-dir="desc"
                                                    {{ $rSort === 'average_rating' ? 'selected' : '' }}>Millor valorats
                                                </option>
                                                <option value="title" data-dir="asc"
                                                    {{ $rSort === 'title' ? 'selected' : '' }}>Ordre alfabètic</option>
                                            </select>
                                            {{-- Camp ocult actualitzat per JS quan canvia el select --}}
                                            <input type="hidden" name="r_direction" id="r_direction_input"
                                                value="{{ $rDirection }}">
                                        </form>
                                    </div>
                                </div>
                                ...
                                <div class="row g-4">
                                    @if(auth()->check() && auth()->id() === $user->id)
                                        ...
                                        <div class="col-md-4">
                                            <div
                                                class="card h-100 d-flex align-items-center justify-content-center profile-create-card-custom">
                                        ...
                                        <div class="col-md-4">
                                            <div style="position: relative;">
                                                    ...
                                                    <div class="card h-100 profile-recipe-card">
                                                        @if($recipe->image)
                                                            <img src="{{ asset('storage/' . $recipe->image) }}"
                                                                alt="{{ $recipe->title }}" class="card-img-top profile-recipe-image">
                                                        @else
                                                            <div class="card-img-top profile-recipe-image d-flex align-items-center justify-content-center"
                                                                style="background: linear-gradient(135deg, #e0e0e0 0%, #f3f3f3 100%); color: #999; font-size: 2rem;">
                                                                <i class="bi bi-image"></i>
                                                            </div>
                                                        @endif
                                                        <div class="card-body">
                                                            <div
                                                                class="d-flex justify-content-between align-items-start gap-2 mb-2 recipe-title-container">
                                                                <h6 class="fw-bold mb-0 profile-recipe-title ">{{ $recipe->title }}
                                                                </h6>
                                                            </div>
                                                            <div class="d-flex justify-content-between align-items-center">
                                                                <span class="text-muted profile-recipe-meta"><i
                                                                        class="bi bi-clock"></i> {{ $recipe->cooking_time }}
                                                                    min</span>
                                                                <span
                                                                    class="text-muted profile-recipe-meta">{{ ucfirst($recipe->difficulty) }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </a>
                                                ...
                                                @if(auth()->check() && auth()->id() === $user->id)
                                                    ...
                                                    <div
                                                        style="position: absolute; top: 12px; right: 12px; display: flex; gap: 8px; z-index: 2;">
                                                        <a href="{{ route('recipes.edit', $recipe) }}" title="Editar recepta"
                                                        ...
                                                        <button type="button" title="Eliminar recepta" aria-label="Eliminar recepta"
                                                        ...
                                                        <form method="POST" action="{{ route('recipes.destroy', $recipe) }}">
                                                            ...
                                                            <div class="modal-header danger-modal-header">
                                                                <h5 class="modal-title text-danger d-flex align-items-center gap-2"
                                                                    id="deleteRecipeModalLabel{{ $recipe->id }}">
                                                            ...
                                                            <div class="modal-body">
                                                                <p class="text-muted small mb-0">
                                                                    Vols eliminar definitivament la recepta
                                                                    <strong>{{ $recipe->title }}</strong>? També desapareixerà del teu
                                                                    rebost i del llistat general de receptes.
                                                                </p>
                                                            </div>
                                        ...
                                        <div class="col-12">
                                            <div class="card profile-recipe-card">
                                                <div class="card-body text-center py-5">
                                                    <h6 class="fw-bold mb-2 profile-recipe-title">Encara no has creat cap recepta
                                                    </h6>
                                                    <p class="text-muted mb-0 profile-recipe-meta">Quan publiquis una recepta,
                                                        apareixerà aquí dins del teu rebost.</p>
                                                </div>
                                            </div>
                                        </div>
                                    @endforelse
                            ...
                            <div class="tab-pane fade {{ $activeProfileTab === 'favorites' ? 'show active' : '' }}"
                                id="profile-favorites-pane" role="tabpanel" aria-labelledby="profile-favorites-tab"
                                tabindex="0">

                                {{-- ── BARRA DE FILTRES DELS FAVORITS ─────────────────────────── --}}
                                @php
                                    $fExceptSearch = request()->except(['f_search', 'f_page', 'tab']);
                                    $fExceptDifficulty = request()->except(['f_difficulty', 'f_page', 'tab']);
                                    $fExceptSort = request()->except(['f_sort', 'f_direction', 'f_page', 'tab']);
                                @endphp
                                    ...
                                    <form method="GET" class="mb-2 search-profile-form">
                                        ...
                                        <div class="input-group input-group-sm">
                                            ...
                                            <input type="text" name="f_search" class="form-control search-profile"
                                                placeholder="Cerca per nom, xef o ingredient..." value="{{ $fSearch }}">
                                            @if($fSearch)
                                            ...
                                            @endif
                                            <button type="submit"
                                                class="btn btn-primary btn-sm profile-search-button bi bi-search"></button>
                                        </div>
                                    </form>
                                    ...
                                    <div class="d-flex flex-wrap gap-2 align-items-center">
                                        ...
                                        <form method="GET" class="ms-auto">
                                            ...
                                            <select name="f_sort" class="form-select form-select-sm d-inline-block w-auto"
                                                onchange="this.form.submit()">
                                                <option value="favorites.created_at" data-dir="desc"
                                                    {{ $fSort === 'favorites.created_at' && $fDirection === 'desc' ? 'selected' : '' }}>
                                                    Afegits més recentment</option>
                                                <option value="favorites.created_at" data-dir="asc"
                                                    {{ $fSort === 'favorites.created_at' && $fDirection === 'asc' ? 'selected' : '' }}>
                                                    Afegits més antigament</option>
                                                <option value="recipes.average_rating" data-dir="desc"
                                                    {{ $fSort === 'recipes.average_rating' ? 'selected' : '' }}>Millor
                                                    valorats</option>
                                                <option value="recipes.title" data-dir="asc"
                                                    {{ $fSort === 'recipes.title' ? 'selected' : '' }}>Ordre alfabètic
                                                </option>
                                            </select>
                                            <input type="hidden" name="f_direction" id="f_direction_input"
                                                value="{{ $fDirection }}">
                                        </form>
                                    </div>
                                </div>
                                    ...
                                    @forelse ($favoriteRecipes as $recipe)
                                        ...
                                        <div class="col-md-4">
                                            <div style="position: relative;">
                                                ...
                                                <form method="POST" action="{{ route('recipes.favorite.destroy', $recipe) }}"
                                                    style="position: absolute; top: 12px; right: 12px; z-index: 2; margin: 0;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <input type="hidden" name="redirect_tab" value="favorites">
                                                    <button type="submit" title="Treure de favorits" aria-label="Treure de favorits"
                                                        style="width: 34px; height: 34px; border-radius: 50%; background: white; color: #BE3144; border: none; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.15); display: flex; align-items: center; justify-content: center;">
                                                        <i class="bi bi-x-lg"></i>
                                                    </button>
                                                </form>
                                        ...
                                        <div class="col-12">
                                            <div class="card profile-recipe-card">
                                                <div class="card-body text-center py-5">
                                                    ...
                                                    <p class="text-muted mb-0 profile-recipe-meta">Quan marquis receptes amb el cor,
                                                        apareixeran aquí.</p>
                                                </div>
                                            </div>
                                        </div>
